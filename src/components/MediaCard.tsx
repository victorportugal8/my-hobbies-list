...
export function MediaCard({ title, type, status, rating, coverUrl }: MediaCardProps) {
    // Mapeamento de ícones por mídia
    const getMediaIcon = () => {
        switch (type) {
    ...
    // Mapeamento de cores e textos por status
    const getStatusConfig = () => {
        switch (status) {
            case 'completed': return { label: 'Concluído', color: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' }
            case 'in_progress': return { label: 'Em Progresso', color: 'bg-blue-500/20 text-blue-400 border-blue-500/30' }
    ...
    // Lógica das estrelas
    const renderStars = () => {
        if(rating === null) return <span className="text-sm text-slate-500 italic">Sem avaliação</span>
        return(
            <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                    <Star key={star} size={16} className={star <= rating ? 'fill-yellow-400 text-yellow-400' : ' fill-transparent text-slate-600'} />
                ))}
            </div>
        )
    }
    ...
    return(
        <div className="group flex flex-col bg-slate-800 rounded-xl overflow-hidden border border-slate-700/50 hover:border-slate-500 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/50 cursor-pointer">
            {/* Capa da mídia */}
